"""Inventory optimizer — non-Markov version.

Translated from Optimizer.m.
"""
import time
import numpy as np
import openpyxl
...
def main():
    ...
    suboptimal_decision_vector_R1 = read_excel_range(
        'Stochastic_Regimes_HL-LL.xlsx', 0, 'B4:B12')
    suboptimal_decision_vector_R2 = read_excel_range(
        'Stochastic_Regimes_HL-LL.xlsx', 0, 'C4:C12')
    suboptimal_decision_matrix_CEC = read_excel_range(
        'Stochastic_Regimes_HL-LL.xlsx', 0, 'F4:T12')
    ...
    # __ PRICE PROCESS __
    k = np.array([[0.99, 0.01], [0.01, 0.99]])
    m = 2
    beliefs = np.array([0.1, 0.3, 0.5, 0.7, 0.9])
    prices = np.array([27.5, 27.5000000001])
    P_pr_reg = np.array([[0.8, 0.1], [0.2, 0.9]])
