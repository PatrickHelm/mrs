...
if (strcmp(advOpt.optimizer,'fminsearch'))||(strcmp(advOpt.optimizer,'fminunc'))
    Spec_Output.Coeff_SE.p=[Spec_Output.Coeff_SE.p ; repmat(nan,1,k)]; 
end

pValue_P=2*(1-tcdf(abs(Spec_Output.Coeff.p)./Spec_Output.Coeff_SE.p,nr-numel(param)));

for i1=1:k
    fprintf(1,'\n      ');
    for i2=1:k
        fprintf(1,'%4.2f (%4.2f,%4.2f)   ',Spec_Output.Coeff.p(i1,i2),Spec_Output.Coeff_SE.p(i1,i2),pValue_P(i1,i2));
    end
end
