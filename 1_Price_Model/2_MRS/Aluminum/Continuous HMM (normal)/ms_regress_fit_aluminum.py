"""Markov Switching Regression for Aluminum prices.

Translated from MS_Regress_Fit_Aluminum.m (based on MS_Regress toolbox by Perlin).
...
def main():
    ...
    log_ret = np.loadtxt(data_dir / 'Aluminum_w.txt')
    ...
    # Model specification
    # MATLAB: k=2, S=[1 0 0 0] → mean switches, variance does NOT switch
    k = 2
    ...
    axes[0].plot(dep)
    axes[0].set_title('Dependent Variable (Aluminum)')
    ...
    plt.tight_layout()
    plt.savefig(data_dir.parent / 'aluminum_regimes.png', dpi=150)
    plt.show()
